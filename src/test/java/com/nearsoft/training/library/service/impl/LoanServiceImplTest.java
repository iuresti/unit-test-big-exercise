...
public class LoanServiceImplTest {
    ...
    public void givenAUserWithLoans_whenLendBooksForAnExistentBook_thenThrowException() {
        ...
        // When:
        assertThatThrownBy(() -> loanService.lendBooks(isbnList))
                .isInstanceOf(LoanNotAllowedException.class)
                .hasMessage("Attempt to borrow more than once a book");

    }


    @Test
    public void givenAUserWithExpiredLoans_whenLendBooksForAnExistentBook_thenThrowException() {
        // Given:
        CardReaderService cardReaderService = Mockito.mock(CardReaderService.class);
        UserService userService = Mockito.mock(UserService.class);
        LoanConfigurationProperties loanConfigurationProperties = new LoanConfigurationProperties();
        ...
        booksByUserForABC.setCurp(curp);
        booksByUserForABC.setIsbn("OBC");
        booksByUserForABC.setBorrowDate(now.minusDays(45));
        ...
        assertThatThrownBy(() -> loanService.lendBooks(isbnList))
                .isInstanceOf(LoanNotAllowedException.class)
                .hasMessage("User must return old loans");
